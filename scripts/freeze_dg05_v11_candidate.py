...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="dg05-v11-bridge-") as raw:
        ...
        done=subprocess.run(cmd,check=True,capture_output=True,text=True); child_receipt=json.loads(done.stdout); bridge=load_self_hashed(child/"V11_BRIDGED_REHEARSAL_RECEIPT.json","dg05_v11_bridged_schedule_rehearsal_v1"); schedule=load_self_hashed(child/"V11_SCHEDULE_ENVELOPE.json","dg05_v11_schedule_execution_envelope_v1"); result=load_self_hashed(child/"V11_STRUCTURAL_RESULT_CONTAINER.json","dg05_v11_structural_result_container_v1")
