...
def _complete_preflight_receipt(
    *,
    approval: Mapping[str, Any],
    replay: Mapping[str, Any],
) -> dict[str, Any]:
    """Make a receipt whose evidence is exclusively the actual replay bundle."""
    if replay.get("status") != "PASS":
        raise DG05V11R2R1RunnerError("COMPLETE_PREFLIGHT_REPLAY_REQUIRED")
    return self_hashed(
        {
            "schema": "dg05_v11r2r1_real_preflight_receipt_v1",
            ...
            "implementation_source_commit": approval["implementation_source_commit"],
            "runtime_approval_replay_hash": approval["self_hash"],
            "complete_replay_bundle_hash": replay["self_hash"],
            "implementation_replay_hash": replay["implementation_replay_hash"],
            "v5_kernel_hash": replay["v5_kernel_hash"],
            "legacy_predecessor_hashes": {
                "legacy": replay["legacy_hash"],
                "v4": replay["v4_hash"],
                "v4_closure": replay["v4_closure_hash"],
                "v1": replay["v1_hash"],
            },
            "physical_preflight_hash": replay["physical_hash"],
            "container_framing_hash": replay["framing_hash"],
            "production_executor_replay_hash": replay["production_executor_hash"],
            "normal_authority_replay_hash": replay["normal_authority_hash"],
            "scenario_authority_hash": replay["scenario_hash"],
            "p1_authority_hash": replay["p1_hash"],
            "source_file_crosswalk_hash": replay["crosswalk_hash"],
            "crosswalk_replay_hash": replay["crosswalk_replay_hash"],
            "execution_scope_id": replay["execution_scope_id"],
            "ledger_status": "UNUSED",
            "heldout_rows_parsed": replay["heldout_rows_parsed"],
            "heldout_predictions": replay["heldout_predictions"],
            "heldout_metrics": replay["heldout_metrics"],
            "execution_consumed": False,
        }
    )


def _verify_stored_preflight(*, receipt_path: Path, approval: Mapping[str, Any], fresh: Mapping[str, Any]) -> dict[str, Any]:
    receipt = load_self_hashed(receipt_path, "dg05_v11r2r1_real_preflight_receipt_v1")
    expected = {
        "release_hash": approval["release_hash"],
        "final_closure_hash": approval["final_closure_hash"],
        "execution_binding_hash": approval["execution_binding_hash"],
        "implementation_replay_hash": fresh["implementation_replay_hash"],
        "v5_kernel_hash": fresh["v5_kernel_hash"],
        "physical_preflight_hash": fresh["physical_hash"],
        "container_framing_hash": fresh["framing_hash"],
        "production_executor_replay_hash": fresh["production_executor_hash"],
        "normal_authority_replay_hash": fresh["normal_authority_hash"],
        "scenario_authority_hash": fresh["scenario_hash"],
        "p1_authority_hash": fresh["p1_hash"],
        "source_file_crosswalk_hash": fresh["crosswalk_hash"],
        "execution_scope_id": fresh["execution_scope_id"],
    }
    if receipt.get("status") != "REAL_PREFLIGHT_PASS_NO_FEATURE_ACCESS" or any(receipt.get(key) != value for key, value in expected.items()):
        raise DG05V11R2R1RunnerError("STORED_PREFLIGHT_FRESH_REPLAY_MISMATCH")
    if receipt.get("ledger_status") != "UNUSED" or receipt.get("execution_consumed") is not False:
        raise DG05V11R2R1RunnerError("STORED_PREFLIGHT_LEDGER_STATUS_REQUIRED")
    if any(receipt.get(key) != 0 for key in ("heldout_rows_parsed", "heldout_predictions", "heldout_metrics")):
        raise DG05V11R2R1RunnerError("STORED_PREFLIGHT_CONTACT_REJECTED")
    return receipt
