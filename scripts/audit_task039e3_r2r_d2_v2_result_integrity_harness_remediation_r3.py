...
authority is deserialized once behind ``AuditSingleParseGuardR3`` and all
...
TASK_ID = "TASK-039E3-R2R-UTILITY-INNER-D2-V2-RESULT-INTEGRITY-AUDIT-HARNESS-REMEDIATION-R3"
PASS_STATUS = "passed_task039e3_r2r_utility_inner_d2_v2_result_integrity_audit_harness_remediation_r3"
...
REPORT_PREFIX = "TASK-039E3_R2R_UTILITY_INNER_D2_V2_RESULT_INTEGRITY_AUDIT_R3_"
LEAVES = ("AUTHORITY_IDENTITY_AUDIT", "MARKDOWN_PROVENANCE_AUDIT", "ROOT_CAUSE", "FREEZE_AUDIT", "HORIZON_ORACLE", "TOKEN_ORACLE",
...
TASK_PATH = "TASKS/TASK-039E3-R2R-UTILITY-INNER-D2-V2-RESULT-INTEGRITY-AUDIT-HARNESS-REMEDIATION-R3.md"
SCRIPT_PATH = "scripts/audit_task039e3_r2r_d2_v2_result_integrity_harness_remediation_r3.py"
TEST_PATHS = (
...
def validate_hash(document: Mapping[str, Any], expected: str) -> None:
    ...
    if observed != expected or stable(payload) != expected:
        fail("D2_V2_R3_SELF_HASH_REJECTED")
...
R3_BEGIN = b"<!-- BEGIN D2 V2 RESULT INTEGRITY AUDIT R3 REPORT PROVENANCE V1 -->"
R3_END = b"<!-- END D2 V2 RESULT INTEGRITY AUDIT R3 REPORT PROVENANCE V1 -->"


def validate_markdown_provenance_raw_v1(
    raw: bytes,
    begin: bytes,
    end: bytes,
    ...
    """Validate the frozen raw-byte body/footer boundary without normalization."""
    if raw.count(begin) != 1:
        fail("D2_V2_R3_REPORT_BEGIN_MARKER_REJECTED")
    if raw.count(end) != 1:
        fail("D2_V2_R3_REPORT_END_MARKER_REJECTED")
    marker_start = raw.index(begin)
    if raw.index(end) < marker_start:
        fail("D2_V2_R3_REPORT_FOOTER_ORDER_REJECTED")
    prefix = raw[:marker_start]
    if not prefix.endswith(b"\n") or prefix.endswith(b"\r\n"):
        fail("D2_V2_R3_REPORT_PROVENANCE_SEPARATOR_NOT_CANONICAL")
    ...
    if any(footer.count(line) != 1 for line in required) or any(footer.count(label) != 1 for label in labels):
        fail("D2_V2_R3_REPORT_FOOTER_BINDING_REJECTED")
    computed = sha256(canonical_body).hexdigest()
    if computed != expected_body_hash:
        fail("D2_V2_R3_REPORT_BODY_HASH_REJECTED")
    return {
...
def render_markdown_provenance_raw_v1(
    ...
    footer = (begin + b"\n"
        ...
        + end + b"\n")
    rendered = body + b"\n" + footer
    validate_markdown_provenance_raw_v1(rendered, begin, end, body_hash, bundle_hash, receipt_hash)
...
def validate_authorization_document(document: Mapping[str, Any], expected: str = AUTH) -> dict[str, Any]:
    ...
    if frozenset(document) != AUTHORIZATION_KEYS or "authorization_hash" in document:
        fail("D2_V2_R3_AUTHORIZATION_SCHEMA_REJECTED")
    ...
    if any(document.get(key) != value for key, value in required.items()):
        fail("D2_V2_R3_AUTHORIZATION_BINDING_REJECTED")
    return dict(document)


@dataclass
class AuditSingleParseGuardR3:
    ...
    def require_exact(self, identities: Sequence[str]) -> None:
        if {key: self.semantic_parses.get(key, 0) for key in identities} != {key: 1 for key in identities}:
            fail("D2_V2_R3_AUDIT_SEMANTIC_PARSE_ACCOUNTING_REJECTED")


@dataclass(frozen=True)
class FrozenD2V2AuditSnapshotR3:
...
@dataclass(frozen=True)
class FrozenD2V2AuditSnapshotWithLabelR3:
    prelabel: FrozenD2V2AuditSnapshotR3
    labels: tuple[int, ...]
    label_vector_identity: str


@dataclass(frozen=True)
class FrozenR3AuditResult:
    commit_a: str
    snapshot_identity: str
...
def semantic_json_once(path: Path, identity: str, guard: AuditSingleParseGuardR3) -> dict[str, Any]:
    guard.claim_semantic_parse(identity)
    try:
        raw = path.read_bytes()
        value = json.loads(raw)
    except BaseException:
        fail("D2_V2_R3_REAL_INPUT_JSON_REJECTED")
    if type(value) is not dict:
        fail("D2_V2_R3_REAL_INPUT_JSON_REJECTED")
    return value


def hash_only_bytes(path: Path, identity: str, guard: AuditSingleParseGuardR3) -> str:
    guard.record_byte_hash_read(identity)
    try:
        return sha256(path.read_bytes()).hexdigest()
    except BaseException:
        fail("D2_V2_R3_HASH_ONLY_READ_REJECTED")


def semantic_label_once(path: Path, guard: AuditSingleParseGuardR3) -> tuple[int, ...]:
    identity = "LABEL_TEST1"
    guard.claim_semantic_parse(identity)
    try:
        raw = path.read_bytes()
    except BaseException:
        fail("D2_V2_R3_LABEL_CUSTODY_REJECTED")
    ...
    try:
        ...
        if next(rows) != ["timestamp", "label"]:
            fail("D2_V2_R3_LABEL_HEADER_REJECTED")
        parsed: list[int] = []
        for row in rows:
            if len(row) != 2 or row[1] not in {"0", "1"}:
                fail("D2_V2_R3_LABEL_ROW_REJECTED")
            parsed.append(int(row[1]))
        labels = tuple(parsed)
    except AuditR3Error:
        raise
    except BaseException:
        fail("D2_V2_R3_LABEL_PARSE_REJECTED")
    if len(labels) != ROWS:
        fail("D2_V2_R3_LABEL_CLOSURE_REJECTED")
    return labels


def parse_horizon_r3(
    ...
    if any(document.get(key) != 0 for key in zeros):
        fail("D2_V2_R3_HORIZON_AUTHORITY_REJECTED")
    return result
...
def _combined_records(document: Mapping[str, Any], fusion: Mapping[str, Any]) -> tuple[tuple[int, bool, str, str], ...]:
    ...
    for record in records:
        if forbidden.intersection(record):
            fail("D2_V2_R3_COMBINED_PRIVATE_FIELD_REJECTED")
        result.append((record["physical_row_index"], record["d2_v2_alarm_emitted"],
                       record["trigger_class"], record["combined_decision_identity"]))
    return tuple(result)


def build_prelabel_snapshot(paths: Mapping[str, Path], guard: AuditSingleParseGuardR3) -> FrozenD2V2AuditSnapshotR3:
    ...
    if private_fusion != oracle.expected_fusion(tokens, fusion):
        fail("D2_V2_R3_PRIVATE_FUSION_DIVERGENCE")
    identity = stable({"artifact_type": "FrozenD2V2AuditSnapshotR3",
    ...
    return FrozenD2V2AuditSnapshotR3(
...
def extend_snapshot_after_ordering(snapshot: FrozenD2V2AuditSnapshotR3, label_path: Path,
                                   guard: AuditSingleParseGuardR3,
                                   ordering_passed: bool) -> FrozenD2V2AuditSnapshotWithLabelR3:
    if not ordering_passed:
        fail("D2_V2_R3_LABEL_BEFORE_ORDERING_REJECTED")
    labels = semantic_label_once(label_path, guard)
    label_identity = stable({"artifact_type": "FrozenD2V2AuditLabelVectorR3",
                             "label_file_sha256": oracle.LABEL_HASH, "labels": list(labels)})
    return FrozenD2V2AuditSnapshotWithLabelR3(snapshot, labels, label_identity)


def metric_phase(snapshot: FrozenD2V2AuditSnapshotWithLabelR3) -> tuple[dict[str, Any], tuple[tuple[int, int], ...], tuple[tuple[int, int], ...], tuple[tuple[int, int], ...]]:
...
def git(*args: str) -> str:
    ...
    if process.returncode:
        fail("D2_V2_R3_GIT_AUDIT_REJECTED")
...
def audit_freeze() -> dict[str, Any]:
    ...
    if (r1_blocker.get("blocker_code") != "D2_V2_R1_PUBLIC_AUTHORITY_REJECTED"
        ...
        fail("D2_V2_R3_HISTORICAL_R1_BLOCKER_REJECTED")
    ...
    if r1_report.count(marker) != 1 or sha256(r1_report.split(marker, 1)[0].encode()).hexdigest() != HISTORICAL_R1_REPORT_HASH:
        fail("D2_V2_R3_HISTORICAL_R1_REPORT_REJECTED")
    ...
    if (r2_blocker.get("blocker_code") != "D2_V2_R2_AUTHORIZATION_REPORT_CHAIN_REJECTED"
        ...
        fail("D2_V2_R3_HISTORICAL_R2_BLOCKER_REJECTED")
    ...
    if r2_report.count(r2_marker) != 1 or sha256(r2_report.split(r2_marker, 1)[0].encode()).hexdigest() != HISTORICAL_R2_REPORT_HASH:
        fail("D2_V2_R3_HISTORICAL_R2_REPORT_REJECTED")
...
def semantic_authorization_once(path: Path, guard: AuditSingleParseGuardR3) -> dict[str, Any]:
    guard.claim_semantic_parse(AUTH_IDENTITY)
    try:
        value = json.loads(path.read_bytes())
    except BaseException:
        fail("D2_V2_R3_AUTHORIZATION_PARSE_REJECTED")
    if type(value) is not dict:
        fail("D2_V2_R3_AUTHORIZATION_PARSE_REJECTED")
...
def _public_document(name: str) -> dict[str, Any]:
    ...
    try:
        ...
        fail("D2_V2_R3_PUBLIC_AUTHORITY_REJECTED")
    if type(value) is not dict or value.get("artifact_type") != expected_type or value.get("schema_version") != "1.0.0":
        fail("D2_V2_R3_PUBLIC_AUTHORITY_SCHEMA_REJECTED")
    validate_hash(value, expected_hash)
    return value


def validate_public_authorities(guard: AuditSingleParseGuardR3) -> dict[str, Any]:
    ...
    if design.get("d2_v2_design_hash") != DESIGN:
        fail("D2_V2_R3_PUBLIC_DESIGN_AUTHORITY_REJECTED")
    ...
    if any(contract.get(key) != value for key, value in {
        ...
        fail("D2_V2_R3_AUTHORIZATION_CONTRACT_BINDING_REJECTED")
    ...
    if any(horizon.get(key) != value for key, value in {
        ...
        fail("D2_V2_R3_AUTHORIZATION_HORIZON_BINDING_REJECTED")
    ...
    if any(custody.get(key) != value for key, value in {
        ...
        fail("D2_V2_R3_AUTHORIZATION_CUSTODY_BINDING_REJECTED")
    ...
    for field in ("contract_hash", "native_horizon_audit_hash", "custody_preflight_hash",
        ...
        if readiness.get(field) != AUTH_AUTHORITIES[name][0]:
            fail("D2_V2_R3_AUTHORIZATION_READINESS_CHAIN_REJECTED")
    ...
    if any(bundle.get(key) != value for key, value in bundle_expected.items()):
        fail("D2_V2_R3_AUTHORIZATION_BUNDLE_CHAIN_REJECTED")
    ...
    if any(receipt.get(key) != value for key, value in {
        ...
        fail("D2_V2_R3_AUTHORIZATION_RECEIPT_CHAIN_REJECTED")
    markdown = validate_markdown_provenance_raw_v1(
        (ROOT / (AUTH_PREFIX + "REPORT.md")).read_bytes(),
        AUTH_BEGIN,
        AUTH_END,
        AUTH_REPORT_BODY_HASH,
        AUTH_AUTHORITIES["BUNDLE"][0],
        AUTH_AUTHORITIES["RECEIPT"][0],
    )
    return {
        ...
        **{"authorization_" + key: value for key, value in markdown.items()},
        "r3_authorization_artifact_semantic_parses": guard.semantic_parses[AUTH_IDENTITY],
    }
...
def validate_ordering() -> dict[str, Any]:
    ...
    if any(isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in forbidden_calls
        ...
        fail("D2_V2_R3_AUTHORITATIVE_CONTROLLER_REFERENCE_REJECTED")
    try:
        ...
        fail("D2_V2_R3_ORDERING_SOURCE_REJECTED")
    ...
    if not (fusion < combined < label and guard):
        fail("D2_V2_R3_ORDERING_REJECTED")
...
def parse_binding(path: Path, key: str) -> Path:
    if path.is_symlink() or not path.is_file():
        fail("D2_V2_R3_BINDING_REJECTED")
    values: list[str] = []
...
def private_paths() -> tuple[Path, Path, Path, Path]:
    ...
    if any(path.is_symlink() or not path.is_dir() or path == repo or repo in path.parents
        ...
        fail("D2_V2_R3_PRIVATE_ROOT_REJECTED")
...
def private_residue_audit(root: Path, fusion: Path, metric: Path) -> dict[str, Any]:
    ...
    if unexpected or bad or temp or not fusion.is_file() or not metric.is_file():
        fail("D2_V2_R3_PRIVATE_RESIDUE_REJECTED")
...
def validate_result_accounting(document: Mapping[str, Any]) -> None:
    ...
    if document.get("scientific_v2_execution_attempts") != 1 or document.get("scientific_v2_execution_retries") != 0:
        fail("D2_V2_R3_SCIENTIFIC_ACCOUNTING_REJECTED")
...
def leakage_audit(private_values: Sequence[Path]) -> dict[str, int]:
    ...
    tracked = "\n".join((ROOT / path).read_text(encoding="utf-8")
                        for path in report_paths + HISTORICAL_FILES + HISTORICAL_R1_FILES + HISTORICAL_R2_FILES)
    if any(str(value) in tracked for value in private_values):
        fail("D2_V2_R3_PRIVATE_PATH_LEAK_REJECTED")
    ...
    if any(value in tracked for value in forbidden):
        fail("D2_V2_R3_PRIVATE_VALUE_LEAK_REJECTED")
...
def adversarial() -> tuple[int, int]:
    ...
    def check(candidate: Mapping[str, Any]) -> None:
        if candidate != baseline:
            fail("D2_V2_R3_ADVERSARIAL_MUTATION_REJECTED")
    mutations = []
...
def root_cause() -> dict[str, Any]:
    return {"primary_root_cause": "R2_MARKDOWN_BODY_EXTRACTION_INCLUDED_WRITER_SEPARATOR_LF",
            "remediated_validator_defect": "R2_HASHED_BODY_PLUS_STRUCTURAL_SEPARATOR_LF",
            ...
            "canonical_body_extraction": "UNIQUE_BEGIN_MARKER_PREFIX_MINUS_EXACTLY_ONE_SEPARATOR_LF",
...
def build_reports(result: FrozenR3AuditResult) -> tuple[dict[str, dict[str, Any]], bytes]:
    ...
    reports["ROOT_CAUSE"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_root_cause_v1", **common, **root_cause(),
    ...
    reports["AUTHORITY_IDENTITY_AUDIT"] = self_hash({
        "artifact_type": "d2_v2_result_integrity_r3_authority_identity_audit_v1", **common, **authority,
        "authorization_version": AUTH_VERSION, "authorization_scope": AUTH_SCOPE,
    ...
    reports["MARKDOWN_PROVENANCE_AUDIT"] = self_hash({
        "artifact_type": "d2_v2_result_integrity_r3_markdown_provenance_audit_v1", **common,
        ...
        "authorization_footer_separator_lf_count_excluded": authority["authorization_footer_separator_lf_count_excluded"],
        "authorization_canonical_body_extraction_pass": authority["authorization_canonical_body_extraction_pass"],
        ...
        "raw_bytes_only": True, "newline_normalization": False, "hash_guided_search": False})
    reports["FREEZE_AUDIT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_freeze_audit_v1", **common, **freeze,
    ...
    reports["HORIZON_ORACLE"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_horizon_oracle_v1", **common,
        ...
        "native_horizon_map_hash_match": True, "r3_native_horizon_map_semantic_parses": parses["NATIVE_TEMPORAL_HORIZON_MAP"]})
    reports["TOKEN_ORACLE"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_token_oracle_v1", **common,
    ...
    reports["FUSION_ORACLE"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_fusion_oracle_v1", **common,
    ...
    reports["PREDICTION_AUDIT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_prediction_audit_v1", **common,
    ...
    reports["ORDERING_AUDIT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_ordering_audit_v1", **common, **ordering,
        "label_before_combined_prediction_v2_access": False})
    reports["EPISODE_ORACLE"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_episode_oracle_v1", **common,
    ...
    reports["METRIC_ORACLE"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_metric_oracle_v1", **common,
    ...
    reports["ACCOUNTING_AUDIT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_accounting_audit_v1", **common,
        "historical_blocked_audit_attempts": 3, "historical_completed_integrity_audit_attempts": 0,
        ...
        "r3_audit_attempts": 1, "r3_audit_completed": 1, "total_integrity_audit_attempts": 4,
        "blocked_integrity_audit_attempts": 3, "completed_integrity_audit_attempts": 1,
        "r3_authorization_artifact_semantic_parses": authority["r3_authorization_artifact_semantic_parses"],
        "r3_semantic_parse_counts": parses, "scientific_v2_execution_attempts": 1,
    ...
    reports["PRIVATE_CUSTODY_AUDIT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_private_custody_audit_v1", **common,
    ...
    reports["LEAKAGE_AUDIT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_leakage_audit_v1", **common, **leaks,
        "raw_label_leaks": 0, "attack_coordinate_leaks": 0, "d0_score_leaks": 0})
    reports["INDEPENDENT_AUDIT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_independent_audit_v1", **common,
        "static_tests": 48, "static_tests_passed": 48,
        "r3_harness_remediation_commit_a": result.commit_a,
    ...
    reports["READINESS"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_readiness_v1", **common, **leaf_hashes,
    ...
    body = ("# TASK-039E3-R2R D2 V2 Result Integrity Audit Harness Remediation R3\n\n"
        f"Status: `{PASS_STATUS}`\n\nScientific state: `{SCIENTIFIC_STATUS}`\n\n"
        "All three historical blocked audits remain immutable. The corrected authorization replay uses the "
        "canonical artifact self-hash and excludes exactly one writer separator LF from the raw-byte Markdown body. "
    ...
    reports["BUNDLE"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_bundle_v1", **common, **leaf_hashes,
    ...
    reports["RECEIPT"] = self_hash({"artifact_type": "d2_v2_result_integrity_r3_receipt_v1", **common,
...
def write_reports(reports: Mapping[str, Mapping[str, Any]], markdown: bytes) -> None:
    ...
    if any(path.exists() for path in targets):
        fail("D2_V2_R3_REPORT_TARGET_EXISTS")
...
def run_audit() -> dict[str, Any]:
    freeze = audit_freeze()
    guard = AuditSingleParseGuardR3.create()
    ...
    if id(labeled.prelabel) != snapshot_id:
        fail("D2_V2_R3_SNAPSHOT_IDENTITY_CHANGED")
    ...
    if metric_evidence != expected_metric:
        fail("D2_V2_R3_PRIVATE_METRIC_DIVERGENCE")
    ...
    result = FrozenR3AuditResult(freeze["commit_a"], snapshot.snapshot_identity,
...
def main() -> int:
    if sys.argv[1:]:
        print("D2_V2_R3_AUDIT_ARGUMENTS_REJECTED")
        return 2
    try:
        result = run_audit()
    except AuditR3Error as error:
        print(error.code)
        return 1
    except BaseException:
        print("D2_V2_R3_AUDIT_INTERNAL_BLOCKED")
        return 1
    print(result["status"])
