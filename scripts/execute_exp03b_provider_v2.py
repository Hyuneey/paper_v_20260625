...
import json
import time
from paperworks.validation_v2.exp03b_semantic_v2 import require,encoded,digest,parse_proposal,proposal_document
from paperworks.validation_v2.exp03b_custody_v1 import publish,seal,replay
from paperworks.validation_v2.exp03b_prompt_v2 import request_body,ENDPOINT
...
from paperworks.validation_v2.exp03b_hidden_v2 import Train2HiddenVerifierAuthorityV2 as Train2HiddenVerifierAuthorityV1,feedback,retrieval,select_t1b,VerifierResultV1
...
from paperworks.validation_v2.exp03b_binder_v2 import require_provider_open
...
def main(approval_path:Path,probe_only:bool):
    require_provider_open(PRIVATE/"provider_execution_v2")
    budget=json.loads((PUBLIC/"EXP03B_PROVIDER_BUDGET_V2.json").read_text());replay(budget)
    freeze=json.loads((PUBLIC/"EXP03B_SEMANTIC_PREPARATION_FREEZE_V2.json").read_text());replay(freeze)
    ...
    require(freeze["status"]=="PREPARED_DG03B_REVISED_PENDING","FULL_PREPARATION_REQUIRED")
    for name,h in freeze['provider_input_hashes'].items():
        require(sha256((PRIVATE/name).read_bytes()).hexdigest()==h,'PROVIDER_INPUT_CUSTODY_CHANGED')
    runroot=PRIVATE/"provider_execution_v2";runroot.mkdir(parents=True,exist_ok=True)
    ...
    try:
        ...
        validate_call_inventory(runroot,budget['maximum_calls'])
        ...
        def call(slot,body):
            ...
            persisted=json.loads((runroot/'calls'/f'{reservation.index:04d}.receipt.json').read_text());replay(persisted)
            require(persisted==seal(receipt),'FIRST_CALL_DURABLE_RECEIPT_REPLAY')
            ...
            if reservation.index==1:
                ...
                persisted_probe=json.loads((runroot/'ONE_CALL_CAPABILITY_RECEIPT.json').read_text());replay(persisted_probe)
                require(persisted_probe['call_receipt_hash']==digest(receipt),'FIRST_CALL_CAPABILITY_REPLAY')
        ...
        for pair in cohort["pairs"]:
            ...
            e=json.loads((PRIVATE/"semantic_v2/train1/provider"/(cid+".json")).read_text())
            hidden=structural(json.loads((PRIVATE/"semantic_v2/train2/structural"/(cid+".json")).read_text()),"train2")
            ids=frozenset(r[7] for r in e["structural_rows"])
        ...
        print(json.dumps({"status":"ALL_PROVIDER_OUTPUTS_FROZEN","next":"SEMANTIC_ADMISSION_TRAIN3_FREEZE_THEN_SCI02B_AND_GUARD_DG04"}))
