...
SCRIPT = ROOT / "scripts/audit_task039e3_r2r_d2_v2_result_integrity_harness_remediation_r1.py"
SPEC = importlib.util.spec_from_file_location("d2_v2_r1_independent", SCRIPT)
...
class IndependentD2V2R1HarnessAuditTests(unittest.TestCase):
    ...
    def test_real_run_is_not_called_by_tests(self) -> None:
        for path in (
            ROOT / "tests/test_task039e3_r2r_d2_v2_result_integrity_audit_harness_remediation_r1.py",
            Path(__file__),
        ):
    ...
    def test_report_schema_closure(self) -> None:
        self.assertEqual(len(subject.REPORT_NAMES), 16)
        self.assertEqual(len(subject.LEAVES), 13)
    ...
    def test_fixed_duplicate_error_is_path_free(self) -> None:
        ...
        try:
            guard.claim_semantic_parse("X")
        except subject.AuditR1Error as error:
            self.assertEqual(str(error), "D2_V2_R1_AUDIT_DUPLICATE_REAL_INPUT_PARSE")
