...
SCRIPT = ROOT / "scripts/audit_task039e3_r2r_d2_v2_result_integrity_harness_remediation_r3.py"
SPEC = importlib.util.spec_from_file_location("d2_v2_r3_independent", SCRIPT)
...
class IndependentD2V2R3HarnessAuditTests(unittest.TestCase):
    ...
    def test_markdown_extraction_is_raw_and_structural(self) -> None:
        tree = ast.parse(SCRIPT.read_text(encoding="utf-8"))
        function = next(node for node in tree.body if isinstance(node, ast.FunctionDef)
                        and node.name == "validate_markdown_provenance_raw_v1")
        rendered = ast.unparse(function)
        self.assertIn("prefix[:-1]", rendered)
        self.assertIn("sha256(canonical_body)", rendered)
        for forbidden in ("rstrip", ".strip(", "decode("):
    ...
    def test_real_run_is_not_called_by_tests(self) -> None:
        for path in (
            ROOT / "tests/test_task039e3_r2r_d2_v2_result_integrity_audit_harness_remediation_r3.py",
            Path(__file__),
        ):
    ...
    def test_fixed_duplicate_error_is_path_free(self) -> None:
        ...
        try:
            guard.claim_semantic_parse("X")
        except subject.AuditR3Error as error:
            self.assertEqual(str(error), "D2_V2_R3_AUDIT_DUPLICATE_REAL_INPUT_PARSE")
