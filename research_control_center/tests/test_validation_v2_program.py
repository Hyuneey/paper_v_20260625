...
class ValidationV2ProgramTests(unittest.TestCase):
    def test_program_state_preserves_v1_and_held_out_boundary(self) -> None:
        ...
        self.assertEqual(
            ...
            'READY_FOR_USER_REAPPROVAL_NO_GO_REAL_ACCESS' if state.get('dg05_executable_v10_closure') or state.get('dg05_executable_v4_closure') else ('DECISION_REQUIRED_NO_GO_REAL_ACCESS' if state.get('dg05_production_chain_closure') else ('DG05_V2_USER_REAPPROVAL_REQUIRED' if state.get('dg05_executable_closure') else ('MULTIPANEL_PRE_DG05_COMPLETE_QA_PASS' if multipanel_frozen else ('XVER_T2_NORMAL_PORTFOLIOS_COMPLETE_QA_PASS' if xver_t2_executed else (
