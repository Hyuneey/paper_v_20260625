...
class Arch009D2FusionTests(unittest.TestCase):
    ...
    def test_miss_recovery_and_generated_summary(self):
        ...
        self.assertTrue("GAP-FIX-001" in summary or "GAP-FIX-002" in summary or "V2-PROTOCOL-001" in summary or "GAP-FIX-METRIC-001" in summary or "EXP-01-EXEC" in summary)

    def test_progression(self):
        state = json.loads((RCC / "registry" / "current_state.yaml").read_text(encoding="utf-8"))
        self.assertTrue(state["last_completed_task"] == "ARCH-011" or state["last_completed_task"].startswith(("GAP-FIX-001", "GAP-FIX-002", "V2-PROTOCOL-001", "GAP-FIX-METRIC-001", "FRESH-MACHINE-SYNTHETIC")))
        self.assertTrue(state["exact_next_task"].startswith(("GAP-FIX-001", "GAP-FIX-002", "V2-PROTOCOL-001", "GAP-FIX-METRIC-001", "EXP-01")))
