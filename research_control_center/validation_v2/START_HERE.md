...
- Historical pre-mapping state recorded HAI22/21 P1 tag·unit·role compatibility as
  `UNRESOLVED`; later normal-only cross-version authorities supersede that current-facing
  status. Those frozen mappings are not changed by this closure task.
...
- Historical experiment/provider gates are preserved in their own records; they
  are not the current operational selector.
...
- Historical receipt state: `NORMAL_ONLY_TRACKS_COMPLETE_EXP04_NEXT` (not the current selector)
