"""TASK-030 structural contracts and migration-assessment foundations."""

from paperworks.contracts.legacy_adapter import (
    ADAPTER_VERSION,
    assess_legacy_artifact,
    assess_legacy_artifact_file,
)
from paperworks.contracts.models import (
    LegacyFieldMapping,
    LegacyMigrationAssessment,
    SchemaRegistration,
    StructuralValidationIssue,
    StructuralValidationReport,
)
from paperworks.contracts.schema_registry import (
    SchemaRegistry,
    SchemaRegistryError,
    load_schema_registry,
    validate_artifact,
    validate_artifact_file,
)
from paperworks.contracts.rule_v1 import (
    AbstentionPolicySpec,
    ComplexitySpec,
    DelayedResponseRuleV1,
    ExpectedEffectSpec,
    LagSpec,
    OperatingRegimeSpec,
    OutputSemanticsSpec,
    PersistenceSpec,
    ReviewHistoryEntry,
    RuleProvenanceSpec,
    RuleV1ModelError,
    SeverityPolicySpec,
    TriggerSpec,
    WindowSpec,
    canonical_rule_document_bytes,
    canonical_rule_document_sha256,
    delayed_response_rule_to_dict,
    load_delayed_response_rule,
    parse_delayed_response_rule,
    serialize_delayed_response_rule,
)
from paperworks.contracts.artifact_hashing import (
    ContractArtifactHashError,
    canonical_contract_artifact_bytes,
    canonical_contract_artifact_sha256,
    verify_contract_artifact_hash,
    with_computed_artifact_hash,
)
from paperworks.contracts.graph_v1 import (
    CandidateGraphV1,
    GraphEdgeV1,
    GraphNodeV1,
    GraphProvenanceV1,
    GraphRangeV1,
    GraphSupportV1,
    GraphUncertaintyV1,
    GraphV1ModelError,
    MetadataProvenanceV1,
    SamplingIntervalV1,
    candidate_graph_to_dict,
    canonical_candidate_graph_sha256,
    load_candidate_graph,
    parse_candidate_graph,
    serialize_candidate_graph,
)
from paperworks.contracts.evidence_v1 import (
    EvidenceLagRangeV1,
    EvidencePackageV1,
    EvidenceSelectionPolicyV1,
    EvidenceV1ModelError,
    MatchedNormalReferenceV1,
    WindowReferenceV1,
    canonical_evidence_package_sha256,
    evidence_package_to_dict,
    load_evidence_package,
    parse_evidence_package,
    serialize_evidence_package,
)
from paperworks.contracts.parameter_v1 import (
    CalibrationParameterV1,
    ParameterConfidenceIntervalV1,
    ParameterSampleSupportV1,
    ParameterStabilityV1,
    ParameterUncertaintyV1,
    ParameterV1ModelError,
    calibration_parameter_to_dict,
    canonical_calibration_parameter_sha256,
    load_calibration_parameter,
    parse_calibration_parameter,
    serialize_calibration_parameter,
)
from paperworks.contracts.phase1_adapters import (
    AdapterFieldMapping,
    ContractAdapterResult,
    DelayedResponseArtifactCollectionV1,
    PHASE1_ADAPTER_VERSION,
    adapt_phase1_calibration_parameter,
    adapt_phase1_candidate_graph,
    adapt_phase1_evidence_package,
)
from paperworks.contracts.accepted_rule import (
    AcceptedRuleError,
    canonical_rule_verification_subject_bytes,
    canonical_rule_verification_subject_sha256,
    materialize_accepted_rule,
)
from paperworks.contracts.verifier_v1 import (
    DelayedResponseVerifierPolicyV1,
    RuleVerificationOutcomeV1,
    VerifierIssueV1,
    VerifierRelationRecordV1,
    VerifierResultV1,
    VerifierStageRecordV1,
    VerifierV1Error,
    canonical_verifier_result_sha256,
    canonical_verifier_binding_id,
    parse_verifier_result,
    serialize_verifier_result,
    verifier_result_to_dict,
    verify_verifier_result_binding,
    verify_delayed_response_rule,
)
from paperworks.contracts.runtime_authority import (
    AUTHORIZATION_VERSION,
    RuntimeAuthorizationBundleV1,
    RuntimeAuthorizationError,
    RuntimeAuthorizationReceiptV1,
    authorization_receipt_to_dict,
    authorize_delayed_response_runtime,
    canonical_runtime_authorization_bytes,
    canonical_runtime_authorization_sha256,
    canonical_verifier_policy_sha256,
    verify_runtime_authorization_bundle,
)
from paperworks.contracts.runtime_v1 import (
    RUNTIME_VERSION,
    DelayedResponseRuntimeWindowV1,
    RuntimeExecutionOutcomeV1,
    RuntimeParameterValueV1,
    RuntimeTraceStepV1,
    RuntimeTraceV1,
    RuntimeV1Error,
    RuntimeWindowModelError,
    canonical_runtime_trace_sha256,
    canonical_runtime_window_bytes,
    canonical_runtime_window_sha256,
    execute_delayed_response_rule,
    load_runtime_window,
    parse_runtime_trace,
    parse_runtime_window,
    runtime_trace_to_dict,
    runtime_window_to_dict,
    serialize_runtime_trace,
)
from paperworks.contracts.explanation_v1 import (
    RENDERER_VERSION,
    ExplanationLagV1,
    ExplanationRecordV1,
    ExplanationResultV1,
    ExplanationTimeIntervalV1,
    ExplanationV1Error,
    canonical_explanation_record_sha256,
    explanation_record_to_dict,
    parse_explanation_record,
    render_delayed_response_explanation,
    serialize_explanation_record,
)
from paperworks.contracts.vertical_slice_v1 import (
    DEFAULT_CONFIG as TASK032F_DEFAULT_CONFIG,
    PIPELINE_VERSION as TASK032F_PIPELINE_VERSION,
    SyntheticAdapterLedgerV1,
    SyntheticRuntimeLedgerV1,
    SyntheticScenarioResultV1,
    SyntheticVerificationLedgerV1,
    SyntheticVerticalSliceError,
    SyntheticVerticalSliceReportV1,
    canonical_vertical_slice_report_bytes,
    canonical_vertical_slice_report_sha256,
    run_task032f_vertical_slice,
    synthetic_vertical_slice_report_to_dict,
    verify_task032f_deterministic_replay,
)

__all__ = [
    "ADAPTER_VERSION",
    "AUTHORIZATION_VERSION",
    "AbstentionPolicySpec",
    "AcceptedRuleError",
    "AdapterFieldMapping",
    "CalibrationParameterV1",
    "CandidateGraphV1",
    "ComplexitySpec",
    "ContractAdapterResult",
    "ContractArtifactHashError",
    "DelayedResponseRuleV1",
    "DelayedResponseRuntimeWindowV1",
    "DelayedResponseArtifactCollectionV1",
    "DelayedResponseVerifierPolicyV1",
    "EvidenceLagRangeV1",
    "EvidencePackageV1",
    "EvidenceSelectionPolicyV1",
    "EvidenceV1ModelError",
    "ExplanationLagV1",
    "ExplanationRecordV1",
    "ExplanationResultV1",
    "ExplanationTimeIntervalV1",
    "ExplanationV1Error",
    "ExpectedEffectSpec",
    "GraphEdgeV1",
    "GraphNodeV1",
    "GraphProvenanceV1",
    "GraphRangeV1",
    "GraphSupportV1",
    "GraphUncertaintyV1",
    "GraphV1ModelError",
    "LagSpec",
    "LegacyFieldMapping",
    "LegacyMigrationAssessment",
    "OperatingRegimeSpec",
    "OutputSemanticsSpec",
    "PHASE1_ADAPTER_VERSION",
    "ParameterConfidenceIntervalV1",
    "ParameterSampleSupportV1",
    "ParameterStabilityV1",
    "ParameterUncertaintyV1",
    "ParameterV1ModelError",
    "PersistenceSpec",
    "ReviewHistoryEntry",
    "RENDERER_VERSION",
    "RUNTIME_VERSION",
    "RuleVerificationOutcomeV1",
    "RuleProvenanceSpec",
    "RuleV1ModelError",
    "RuntimeAuthorizationBundleV1",
    "RuntimeAuthorizationError",
    "RuntimeAuthorizationReceiptV1",
    "RuntimeExecutionOutcomeV1",
    "RuntimeParameterValueV1",
    "RuntimeTraceStepV1",
    "RuntimeTraceV1",
    "RuntimeV1Error",
    "RuntimeWindowModelError",
    "SchemaRegistration",
    "SchemaRegistry",
    "SchemaRegistryError",
    "StructuralValidationIssue",
    "StructuralValidationReport",
    "SyntheticAdapterLedgerV1",
    "SyntheticRuntimeLedgerV1",
    "SyntheticScenarioResultV1",
    "SyntheticVerificationLedgerV1",
    "SyntheticVerticalSliceError",
    "SyntheticVerticalSliceReportV1",
    "TASK032F_DEFAULT_CONFIG",
    "TASK032F_PIPELINE_VERSION",
    "SamplingIntervalV1",
    "SeverityPolicySpec",
    "TriggerSpec",
    "VerifierIssueV1",
    "VerifierRelationRecordV1",
    "VerifierResultV1",
    "VerifierStageRecordV1",
    "VerifierV1Error",
    "WindowSpec",
    "WindowReferenceV1",
    "MatchedNormalReferenceV1",
    "MetadataProvenanceV1",
    "adapt_phase1_calibration_parameter",
    "adapt_phase1_candidate_graph",
    "adapt_phase1_evidence_package",
    "assess_legacy_artifact",
    "assess_legacy_artifact_file",
    "canonical_rule_document_bytes",
    "canonical_rule_document_sha256",
    "canonical_rule_verification_subject_bytes",
    "canonical_rule_verification_subject_sha256",
    "canonical_runtime_authorization_bytes",
    "canonical_runtime_authorization_sha256",
    "canonical_runtime_trace_sha256",
    "canonical_runtime_window_bytes",
    "canonical_runtime_window_sha256",
    "canonical_contract_artifact_bytes",
    "canonical_contract_artifact_sha256",
    "canonical_candidate_graph_sha256",
    "canonical_evidence_package_sha256",
    "canonical_calibration_parameter_sha256",
    "canonical_verifier_result_sha256",
    "canonical_verifier_binding_id",
    "canonical_verifier_policy_sha256",
    "canonical_vertical_slice_report_bytes",
    "canonical_vertical_slice_report_sha256",
    "canonical_explanation_record_sha256",
    "calibration_parameter_to_dict",
    "authorization_receipt_to_dict",
    "authorize_delayed_response_runtime",
    "candidate_graph_to_dict",
    "delayed_response_rule_to_dict",
    "load_delayed_response_rule",
    "load_calibration_parameter",
    "load_candidate_graph",
    "load_evidence_package",
    "load_schema_registry",
    "materialize_accepted_rule",
    "parse_delayed_response_rule",
    "parse_calibration_parameter",
    "parse_candidate_graph",
    "parse_evidence_package",
    "parse_verifier_result",
    "evidence_package_to_dict",
    "execute_delayed_response_rule",
    "explanation_record_to_dict",
    "load_runtime_window",
    "parse_explanation_record",
    "parse_runtime_trace",
    "parse_runtime_window",
    "render_delayed_response_explanation",
    "runtime_trace_to_dict",
    "runtime_window_to_dict",
    "run_task032f_vertical_slice",
    "serialize_delayed_response_rule",
    "serialize_calibration_parameter",
    "serialize_candidate_graph",
    "serialize_evidence_package",
    "serialize_explanation_record",
    "serialize_runtime_trace",
    "serialize_verifier_result",
    "validate_artifact",
    "validate_artifact_file",
    "verify_contract_artifact_hash",
    "verify_runtime_authorization_bundle",
    "verify_task032f_deterministic_replay",
    "verifier_result_to_dict",
    "verify_verifier_result_binding",
    "verify_delayed_response_rule",
    "with_computed_artifact_hash",
    "synthetic_vertical_slice_report_to_dict",
]
