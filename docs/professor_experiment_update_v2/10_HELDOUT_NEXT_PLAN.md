...
# DG-05 Executable V3 사전 접근 폐쇄

DG-05 V2 재승인은 결과 builder와 독립 verifier가 전체 사전등록 지표 표면을 재현하지 못해 Phase A 전에 중단되었으며, 실제 attack/test·label/scenario 접근은 0입니다. V3는 기존 과학 정의를 바꾸지 않고, 연결된 합성 production 경로(72 prediction cells → global freeze → 단일 lease/custodian → 146 scenarios/denominator → complete builder → path-only verifier)에서 228개 typed metric surface 전부를 재계산했습니다. 12개 mutation class와 surface별 omission/authority-binding 228건씩이 모두 거부되었습니다.

DEC-030/V2는 미실행 역사 기록으로 보존합니다. 실제 held-out 접근에는 새 exact-hash `DG-05 REAPPROVAL — EXECUTABLE V3`가 필요합니다. 이 폐쇄는 실행 준비 결과이지 공격 성능 또는 일반화 결과가 아닙니다. 교수 package는 제출하지 않았습니다.
