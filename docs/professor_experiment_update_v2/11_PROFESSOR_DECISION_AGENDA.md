...
해당 사전 동결 작업은 완료되었습니다. 정확한 다음 gate는 DG-05이며, 승인 전에는 attack feature·label·scenario authority에 접근하지 않습니다. 백업은 SINGLE_COPY_LOCAL_ONLY입니다.
...
# 현재 결정 안건 — DG-05 Executable V3 재승인

Executable V2는 미실행 상태에서 complete metric-surface 결손으로 중단되었습니다. V3 closure는 동결 과학 계약을 유지한 채 연결된 72-cell prediction/freeze/lease/custodian/146-scenario 경로에서 228/228 builder/verifier surface를 재계산했고, 12/12 mutation class 및 surface별 omission/authority-binding 228/228을 거부했습니다. 실제 attack/test·label/scenario 접근은 0이며, DEC-030은 재사용하지 않습니다.

다음 연구자 결정은 새 V3 manifest/closure hash에 대한 명시적 DG-05 재승인입니다. 이 package 자체는 제출되지 않았고 DG-06은 별도입니다.
...
1. 두 fixed fusion이 Recall 개선 없이 FAR를 늘린 음성 결과의 해석과 thesis 범위.
2. 높은 Rule-only FAR를 현재 한계로 명시하고 개발 결과를 최종 검증과 구분.
3. DG-03 고정 snapshot 실행 완료. EXP-03 결과를 바탕으로 DG-04 제목·기여 표현 결정.
4. DG-04: GDN-Assisted supporting evidence 표현과 조건부 Agentic 제목 검토.
5. 14개 test1 시나리오를 인위적으로 세분하지 않고 개발 근거로만 보존하는 해석.
6. DG-05: HAI 23.05 test2를 primary held-out으로, HAI 22.04/21.03을 version-separated external replication으로 승인할지 검토. 현재 신규 attack 접근은 0.
7. P1 eligibility를 prediction 이전에 독립 custodian이 고정하고, official scenario를 주 통계 단위로 사용할지 검토.
